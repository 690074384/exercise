...
import { MYROOM_SUMMARY_SUCCEEDED, MYROOM_SUMMARY_FAILED,
            MYROOM_DELETE_SUCCEEDED, MYROOM_DELETE_FAILED,
            RECALL_GRAB_SUCCEEDED, RECALL_GRAB_FAILED,
         } from '../constants/myRoom';
...
export default handleActions({
    ...
    [RECALL_GRAB_SUCCEEDED](state, action) {
        Toast.hide();
        const { deleteId } = action;
        //得到被删除的预定的id
        //手动删除state中会议室列表中对应的会议
    ...
    [RECALL_GRAB_FAILED](state, action) {
        Toast.hide();
        Toast.info(action.message);
        return state;
    },
